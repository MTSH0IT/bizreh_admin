import 'package:bizreh_admin/features/payment/controllers/user_payments_and_orders_controller.dart';
import 'package:bizreh_admin/utils/widgets/app_form_dialog.dart';
import 'package:bizreh_admin/utils/widgets/form_dialog_actions.dart';
import 'package:bizreh_admin/utils/widgets/labeled_text_field.dart';
import 'package:bizreh_admin/utils/widgets/loading_dropdown_form_field2.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:get/get.dart';

class UserAddPaymentFormDialog extends StatelessWidget {
  final UserPaymentsAndOrdersController controller;

  const UserAddPaymentFormDialog({super.key, required this.controller});

  List<DropdownMenuItem<String>> _buildPaymentTypeItems() {
    return const [
      DropdownMenuItem<String>(value: 'regular', child: Text('Regular')),
      DropdownMenuItem<String>(value: 'bonus', child: Text('Bonus')),
    ];
  }

  @override
  Widget build(BuildContext context) {
    return AppFormDialog(
      title: const Text('Create Payment'),
      actions: [
        FormDialogActions(
          onCancel: () {
            controller.clearPaymentForm();
            Get.back();
          },
          onSubmit: controller.createPaymentForSelectedUser,
          isBusy: () => controller.isCreatingPayment.value,
          submitText: 'Create',
        ),
      ],
      child: Form(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            LabeledTextField(
              label: 'Amount',
              hint: 'Enter amount',
              controller: controller.paymentAmountController,
              keyboardType: const TextInputType.numberWithOptions(
                decimal: true,
              ),
              inputFormatters: [
                FilteringTextInputFormatter.allow(RegExp(r'^\d+\.?\d{0,2}')),
              ],
            ),
            const SizedBox(height: 16),
            LoadingDropdownFormField2<String>(
              isLoading: false,
              labelText: 'Payment Type',
              hintText: 'Select payment type',
              value: controller.paymentTypeController.text.isNotEmpty
                  ? controller.paymentTypeController.text
                  : null,
              onChanged: (value) {
                if (value != null) {
                  controller.paymentTypeController.text = value;
                }
              },
              items: _buildPaymentTypeItems(),
            ),
            const SizedBox(height: 16),
            LabeledTextField(
              label: 'Notes',
              hint: 'Enter notes (optional)',
              controller: controller.paymentNotesController,
              maxLines: 3,
            ),
          ],
        ),
      ),
    );
  }
}
